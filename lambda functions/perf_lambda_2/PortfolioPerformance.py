...
class Investment_Analysis:
    ...
    def __init__(self,BasketCompostion,BaseCurrency,api_key,start_date=dt.datetime.today() - dt.timedelta(days=7),end_date=dt.datetime.today()):
        """
            :Parameters:
                ...
                api_key : str
                    API KEY for Rapid API Yahoo Finance
    ...
    def _getEquitiesHistory(self):
        """Uses the yfinance module to get pricing data for each equity returns a dataframe of prices"""
        ...
        for id in self.BasketComposition:
            ...
            equity_history = equity.history(start=self.start_date,end=self.end_date)
            equity_history.reset_index(inplace=True)
